...
export const Brincos = forwardRef((props, ref) => {
  ...
  // Filtrar produtos do tipo 'brinco'
  const brincoProducts = products.filter(product => product.type.toLowerCase() === 'brinco');

  // Filtrar produtos que correspondem diretamente ao termo de busca
  const directMatches = brincoProducts.filter((product) =>
    product.title.toLowerCase().includes(searchInput)
  );

  // Filtrar produtos que têm uma similaridade com o termo de busca
  const similarMatches = brincoProducts.filter((product) =>
