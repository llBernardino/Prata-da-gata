...
export const Pulseiras = forwardRef((props, ref) => {
  ...
  // Filtrar produtos do tipo 'Pulseira'
  const pulseiraProducts = products.filter(product => product.type.toLowerCase() === 'pulseira');

  // Filtrar produtos que correspondem diretamente ao termo de busca
  const directMatches = pulseiraProducts.filter((product) =>
    product.title.toLowerCase().includes(searchInput)
  );

  // Filtrar produtos que têm uma similaridade com o termo de busca
  const similarMatches = pulseiraProducts.filter((product) =>
  ...
  return (
    <Content className='mt-5 pt-5'>
      <Backgroundproducts ref={ref}>
        <Container>
          <H1 className="text-center">NOVIDADES! COMPRE JÁ - PULSEIRAS:</H1>
